...
namespace Microsoft.Azure.Documents.Query
{
	...
	internal sealed class OrderByContinuationToken
	{
		...
		public OrderByContinuationToken(CompositeContinuationToken compositeContinuationToken, QueryItem[] orderByItems, string rid, int skipCount, string filter)
		{
			if (compositeContinuationToken == null)
			{
				throw new ArgumentNullException(string.Format("{0} can not be null.", "compositeContinuationToken"));
			}
			...
			{
				throw new ArgumentNullException(string.Format("{0} can not be null or empty or whitespace.", "rid"));
			}
			if (skipCount < 0)
			{
				throw new ArgumentException(string.Format("{0} can not be negative.", "skipCount"));
			}
		...
		{
			if (!TryParse(value, out OrderByContinuationToken orderByContinuationToken))
			{
		...
		public static bool TryParse(string value, out OrderByContinuationToken orderByContinuationToken)
		{
			...
			{
				DefaultTrace.TraceWarning(string.Format("{0} Invalid continuation token {1} for Top~Component, exception: {2}", DateTime.UtcNow.ToString("o", CultureInfo.InvariantCulture), value, ex.Message));
				return false;
			}
		}
	}
