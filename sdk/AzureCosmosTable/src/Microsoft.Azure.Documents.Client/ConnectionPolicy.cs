...
namespace Microsoft.Azure.Documents.Client
{
	...
	public sealed class ConnectionPolicy
	{
		...
		/// </para>
		/// </remarks>
		public Collection<string> PreferredLocations => preferredLocations;
		...
		/// first region in <see cref="P:Microsoft.Azure.Documents.Client.ConnectionPolicy.PreferredLocations" /> property.</value>
		/// </remarks>
		...
		internal event NotifyCollectionChangedEventHandler PreferenceChanged
		{
			add
			{
				preferredLocations.CollectionChanged += value;
			}
			remove
			{
				preferredLocations.CollectionChanged -= value;
		...
		public ConnectionPolicy()
		{
			...
			MediaRequestTimeout = TimeSpan.FromSeconds(300.0);
			ConnectionMode = ConnectionMode.Gateway;
			MaxConcurrentFanoutRequests = 32;
			MediaReadMode = MediaReadMode.Buffered;
			UserAgentContainer = new UserAgentContainer();
			preferredLocations = new ObservableCollection<string>();
			EnableEndpointDiscovery = true;
			MaxConnectionLimit = 50;
			RetryOptions = new RetryOptions();
			EnableReadRequestsFallback = null;
		}
		...
		public void SetCurrentLocation(string location)
		{
			...
			List<string> list = RegionProximityUtil.GeneratePreferredRegionList(location);
			if (list != null)
			{
				preferredLocations.Clear();
				foreach (string item in list)
				{
					preferredLocations.Add(item);
				}
			}
		}
	}
