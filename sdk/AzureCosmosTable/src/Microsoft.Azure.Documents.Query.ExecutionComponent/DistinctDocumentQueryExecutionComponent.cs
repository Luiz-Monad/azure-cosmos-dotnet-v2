	...
	internal sealed class DistinctDocumentQueryExecutionComponent : DocumentQueryExecutionComponentBase
	{
		...
		private struct DistinctContinuationToken
		{
			...
			private class NullableUInt192Serializer : JsonConverter
			{
				...
				{
					if ((object)objectType != typeof(UInt192))
					{
				...
				public override void WriteJson(JsonWriter writer, object value, JsonSerializer serializer)
				{
					...
					{
						JToken.FromObject(((UInt192)value).ToString()).WriteTo(writer);
					}
				}
			}
			...
			public static DistinctContinuationToken Parse(string value)
			{
				if (!TryParse(value, out DistinctContinuationToken distinctContinuationToken))
				{
			...
			public static bool TryParse(string value, out DistinctContinuationToken distinctContinuationToken)
			{
				...
				{
					DefaultTrace.TraceWarning(string.Format("{0} Invalid continuation token {1} for Distinct~Component, exception: {2}", DateTime.UtcNow.ToString("o", CultureInfo.InvariantCulture), value, ex.Message));
					return false;
				}
			}
		...
		private DistinctDocumentQueryExecutionComponent(DistinctQueryType distinctQueryType, UInt192? previousHash, IDocumentQueryExecutionComponent source)
			: base(source)
		{
			...
			distinctMap = DistinctMap.Create(distinctQueryType, previousHash);
		}
		...
		public static async Task<IDocumentQueryExecutionComponent> CreateAsync(string requestContinuation, Func<string, Task<IDocumentQueryExecutionComponent>> createSourceCallback, DistinctQueryType distinctQueryType)
		{
			...
			if (requestContinuation != null)
			{
				...
				if (distinctQueryType != DistinctQueryType.Ordered && distinctContinuationToken.LastHash.HasValue)
				{
			...
			UInt192? previousHash = distinctContinuationToken.LastHash;
			return new DistinctDocumentQueryExecutionComponent(distinctQueryType, previousHash, await createSourceCallback(distinctContinuationToken.SourceToken));
		}
		...
		public override async Task<FeedResponse<object>> DrainAsync(int maxElements, CancellationToken cancellationToken)
		{
			...
			{
				JToken jTokenFromObject = GetJTokenFromObject(item);
				if (distinctMap.Add(jTokenFromObject, out lastHash))
				{
			...
			if (!IsDone)
			{
				...
				feedResponse.ResponseContinuation = new DistinctContinuationToken(lastHash, responseContinuation).ToString();
			}
			else
			{
				Source.Stop();
			...
			return new FeedResponse<object>(distinctResults, distinctResults.Count, feedResponse.Headers, feedResponse.UseETagAsContinuation, feedResponse.QueryMetrics, feedResponse.RequestStatistics, (distinctQueryType == DistinctQueryType.Ordered) ? null : RMResources.UnorderedDistinctQueryContinuationToken, feedResponse.ResponseLengthBytes);
		}
