...
namespace Microsoft.Azure.Documents
{
	...
	internal struct SchedulingTimeSpan
	{
		...
		/// </summary>
		public TimeSpan WaitTime => waitTime;
		...
		{
			return GetAverageTime(schedulingTimeSpans, (SchedulingTimeSpan schedulingMetric) => schedulingMetric.TurnaroundTime.Ticks);
		}
		...
		{
			return GetAverageTime(schedulingTimeSpans, (SchedulingTimeSpan schedulingMetric) => schedulingMetric.ResponseTime.Ticks);
		}
		...
		{
			return GetAverageTime(schedulingTimeSpans, (SchedulingTimeSpan schedulingMetric) => schedulingMetric.RunTime.Ticks);
		}
		...
		{
			long ticks = schedulingTimeSpans.Sum((SchedulingTimeSpan schedulingMetric) => schedulingMetric.TurnaroundTime.Ticks);
			TimeSpan timeSpan = new TimeSpan(ticks);
			return (double)schedulingTimeSpans.Count() / timeSpan.TotalSeconds;
		}
		...
		{
			long num = schedulingTimeSpans.Max((SchedulingTimeSpan schedulingMetric) => schedulingMetric.TurnaroundTime.Ticks);
			return (double)Convert.ToInt64(schedulingTimeSpans.Sum((SchedulingTimeSpan schedulingMetric) => schedulingMetric.RunTime.Ticks)) / (double)num;
		}
		...
		{
			return string.Format(CultureInfo.InvariantCulture, "Turnaround Time: {0}ms, Response Time: {1}ms, Run Time: {2}ms, Wait Time: {3}ms, Number Of Preemptions: {4}", TurnaroundTime.TotalMilliseconds, ResponseTime.TotalMilliseconds, RunTime.TotalMilliseconds, WaitTime.TotalMilliseconds, NumPreemptions);
		}
		...
		private static TimeSpan GetAverageTime(IEnumerable<SchedulingTimeSpan> schedulingTimeSpans, Func<SchedulingTimeSpan, long> propertySelectorCallback)
		{
			...
			return new TimeSpan(Convert.ToInt64(schedulingTimeSpans.Average(propertySelectorCallback)));
		}
	}
}
